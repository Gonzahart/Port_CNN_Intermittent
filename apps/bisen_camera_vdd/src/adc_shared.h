...
// direct-VDD monitor (the internal BATT channel). Both must share ADC0; a
...
typedef enum {
    ...
    ADC_MODE_SUPPLY,      // slot 1 = internal VDD/3, LPMODE1 (53.7 us, lower power)
...
// templates, then enters ADC_MODE_PIXEL. The supply channel is internal and
// requires no external pin or RC-settle delay. Call once, from sensor_init().
...
// One robust direct-VDD decision sample. Switches to ADC_MODE_SUPPLY,
...
// Nominal conversion documented by AmbiqSuite R4.5.0:
// BATT is the internal VDD/3 channel and the Apollo4 Plus ADC reference is
// AM_HAL_ADC_VREFMV (1190 mV). This is for bring-up only; policy thresholds
// require a DMM fit on the physical board first.
uint32_t adc_shared_supply_nominal_millivolts(uint32_t code);

// Read-only diagnostic for MCUCTRL->ADCBATTLOAD. This app never enables that
// optional load resistor; sampling the BATT channel does not require doing so.
uint32_t adc_shared_battload_register(void);
