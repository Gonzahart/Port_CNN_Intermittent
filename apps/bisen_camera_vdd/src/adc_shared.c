...
#error "bisen_camera_vdd stage 1 requires the Apollo4 Plus internal BATT channel"
#endif
...
// R4.5.0 defines BATT as the Apollo4 Plus internal voltage divide-by-3
// connection. It consumes no GPIO, so GPIO16 is deliberately unused here and
// GPIO15/SE4 remains the photodiode input.
#define SUPPLY_CHANNEL  AM_HAL_ADC_SLOT_CHSEL_BATT
#define SUPPLY_SLOT     1

// Runtime supply decisions switch ADC0 between the camera's SE4 input and the
// internal BATT channel. The hardware AVG16 result removes ordinary conversion
...
uint32_t adc_shared_supply_nominal_millivolts(uint32_t code) {
    // R4.5.0 am_hal_adc.h: BATT is VDD/3, VREF=1190 mV, 12-bit divisor=4096.
    const uint64_t numerator =
        (uint64_t)code * (uint64_t)AM_HAL_ADC_VREFMV * 3u;
    return (uint32_t)((numerator + AM_HAL_ADC_SAMPLE_DIVISOR / 2u) /
                      AM_HAL_ADC_SAMPLE_DIVISOR);
}

uint32_t adc_shared_battload_register(void) {
    return MCUCTRL->ADCBATTLOAD;
}
