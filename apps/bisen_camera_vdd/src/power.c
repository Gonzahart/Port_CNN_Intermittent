...
#else
#define UA_SETTLE UA_SPIN_HP
#endif

static const uint32_t s_ua[PWR_COUNT] = {
    [PWR_OTHER]   = UA_ACTIVE_HP,
    [PWR_SETTLE]  = UA_SETTLE,
    [PWR_ADC]     = UA_ACTIVE_HP + UA_ADC_ADDER,  // trigger, poll, FIFO read
    [PWR_ADCWAIT] = UA_SETTLE,                    // slept, so priced as sleep
    [PWR_INFER]   = UA_ACTIVE_HP,
    [PWR_UART]    = UA_SPIN_HP,                   // blocks on the UART FIFO
