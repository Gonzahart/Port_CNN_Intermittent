...
int infer_init(void) {
    ...
    // BSS starts at layer 0/unit 0, which is a valid in-progress position.
    // An absent or rejected checkpoint must therefore be made explicitly
    // inactive; otherwise workload state inspection can mistake a camera scan
    // for an inference checkpoint at position zero.
    nn_abandon(&g_net);
    return 0;
}
