...
#ifndef CKPT_MAGIC
#define CKPT_MAGIC 0x48565231u  /* "HVR1": never adopt old trace checkpoints */
#endif
...
// Continuous replay has a separate, two-slot control journal. It records only
// whether the user has armed continuous execution; workload progress remains
// in the larger checkpoint slots above. Keeping the two records independent
// lets a completed job retire its payload without forgetting that the next job
// should start, and lets BTN1 park the system without discarding resumable
// progress.
#define CKPT_SESSION_NUM_SLOTS 2u
#define CKPT_SESSION_RECORD_SZ CKPT_ALIGN
...
// Durable continuous-run control. A fresh/erased image is disarmed. Each
// change is committed as one 16-byte MRAM program into the alternate slot.
// `ckpt_session_set_armed` returns 0 on success (including no change) and -1
