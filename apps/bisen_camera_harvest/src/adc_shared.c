...
#endif
#ifndef BISEN_HARVEST_SENSE_ENABLE_PIN
#define BISEN_HARVEST_SENSE_ENABLE_PIN 0
#endif
#ifndef BISEN_HARVEST_SENSE_ACTIVE_HIGH
#define BISEN_HARVEST_SENSE_ACTIVE_HIGH 1
#endif
#ifndef BISEN_HARVEST_SENSE_SETTLE_US
#define BISEN_HARVEST_SENSE_SETTLE_US 500
#endif
#ifndef BISEN_HARVEST_PARK_ADC
#define BISEN_HARVEST_PARK_ADC 1
#endif
    ...
    (BISEN_HARVEST_SENSE_ENABLE_PIN == 0 || \
     ...
     BISEN_HARVEST_SENSE_ENABLE_PIN == 35 || \
     BISEN_HARVEST_SENSE_ENABLE_PIN == 36 || \
     BISEN_HARVEST_SENSE_ENABLE_PIN == 61 || \
     BISEN_HARVEST_SENSE_ENABLE_PIN == 62 || \
     BISEN_HARVEST_SENSE_ENABLE_PIN == 63 || \
     BISEN_HARVEST_SENSE_ENABLE_PIN == 100)
#error "Select an unused, verified GPIO for the divider switch"
#endif

static void sense_power(int on) {
#if BISEN_HARVEST_SWITCHED_DIVIDER
    const int high = on ? BISEN_HARVEST_SENSE_ACTIVE_HIGH :
                          !BISEN_HARVEST_SENSE_ACTIVE_HIGH;
    am_hal_gpio_state_write(BISEN_HARVEST_SENSE_ENABLE_PIN,
        high ? AM_HAL_GPIO_OUTPUT_SET : AM_HAL_GPIO_OUTPUT_CLEAR);
    if (on) am_util_delay_us(BISEN_HARVEST_SENSE_SETTLE_US);
#else
    (void)on;
#endif
}
...
static void resume_if_parked(void) {
...
#if BISEN_HARVEST_SWITCHED_DIVIDER
    // An external pull must hold the high-side switch OFF through reset.
    sense_power(0);
    am_hal_gpio_pincfg_t sense_out = AM_HAL_GPIO_PINCFG_OUTPUT;
    (void)am_hal_gpio_pinconfig(BISEN_HARVEST_SENSE_ENABLE_PIN, sense_out);
    sense_power(0);
#endif
...
#if BISEN_HARVEST_PARK_ADC
    if (!g_h || g_parked) return;
    sense_power(0);
    (void)am_hal_adc_disable(g_h);
    (void)am_hal_adc_power_control(g_h, AM_HAL_SYSCTRL_DEEPSLEEP, false);
    g_parked = 1;
#endif
}
