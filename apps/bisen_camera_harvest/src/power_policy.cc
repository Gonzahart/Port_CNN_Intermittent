...
static void bus_check(uint8_t intended) {
    harvest_stats_note_state(intended);
    s_bus_marks++;
...
#else
static void bus_check(uint8_t intended) { harvest_stats_note_state(intended); }
#endif
...
extern "C" int wl_supply_mv(uint32_t *out_mv) {
    ...
    *out_mv = bisen::calibrated_vcap_millivolts_from_code(code);
    return 0;
#elif ES_SOURCE == ES_SOURCE_BATT || ES_SOURCE == ES_SOURCE_TRACE || \
      ES_SOURCE == ES_SOURCE_HARVEST
    if (out_mv == 0) return -1;
    uint32_t code;
