...
static uint32_t g_session_seq;
static uint32_t g_session_next_slot;
static int g_session_armed;

#define CKPT_SESSION_MAGIC (CKPT_MAGIC ^ 0x00001100u) /* HVRx -> HVCx */
...
typedef struct {
    ...
    uint32_t armed;
    uint32_t crc;
} ckpt_session_record_t;

typedef char ckpt_session_record_is_one_program[
    sizeof(ckpt_session_record_t) == CKPT_SESSION_RECORD_SZ ? 1 : -1];
...
static int read_session_record(uint32_t slot, ckpt_session_record_t *record) {
    if (ckpt_dev_session_read(slot, record) != 0) return 0;
    if (record->magic != CKPT_SESSION_MAGIC || record->armed > 1u) return 0;
    return record->crc ==
           crc32(record, sizeof(*record) - sizeof(record->crc), 0u);
}
...
void ckpt_init(void) {
    ...
    g_session_seq = 0u;
    g_session_next_slot = 0u;
    g_session_armed = 0;
    int session_found = -1;
    for (uint32_t s = 0; s < CKPT_SESSION_NUM_SLOTS; ++s) {
        ckpt_session_record_t record;
        if (read_session_record(s, &record) &&
            (session_found < 0 || record.seq > g_session_seq)) {
            g_session_seq = record.seq;
            g_session_armed = record.armed != 0u;
            session_found = (int)s;
        }
    }
    if (session_found >= 0) {
        g_session_next_slot =
            ((uint32_t)session_found + 1u) % CKPT_SESSION_NUM_SLOTS;
    }
}

int ckpt_session_is_armed(void) {
    return g_session_armed;
}

int ckpt_session_set_armed(int armed) {
    armed = armed != 0;
    if (armed == g_session_armed) return 0;

    ckpt_session_record_t record;
    record.magic = CKPT_SESSION_MAGIC;
    record.seq = g_session_seq + 1u;
    record.armed = (uint32_t)armed;
    record.crc = crc32(&record, sizeof(record) - sizeof(record.crc), 0u);
    if (ckpt_dev_session_write(g_session_next_slot, &record) != 0) return -1;

    g_session_seq = record.seq;
    g_session_armed = armed;
    g_session_next_slot =
        (g_session_next_slot + 1u) % CKPT_SESSION_NUM_SLOTS;
    return 0;
}
