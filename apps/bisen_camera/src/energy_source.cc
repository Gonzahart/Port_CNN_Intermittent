...
// The levels are derived from HIS thresholds, not from new numbers of ours.
// A source that reports millivolts hands them to his band policy as well; this
// is only so that a source WITHOUT a voltage can still say something useful.
...
//
// ⚠ IT READS THE REGULATED RAIL, not the storage capacitor. That rail is flat
// while the regulator holds, so this gives a LATE warning -- by the time it
// moves, the reserve may already be gone. It is here because it costs nothing
// to try and is worth measuring, not because it is a substitute for tapping the
// capacitor ahead of the regulator.
//
// Still needs the same shared-ADC work as ES_SOURCE_VCAP: a second slot on
// sensor.c's existing instance, not a second owner.
...
uint32_t es_read_cost_us(void) { return 107u; }   // one conversion, no settle
void es_set_load_uw(uint32_t microwatts) { (void)microwatts; }
const char *es_source_name(void) { return "internal BATT channel"; }
