...
constexpr uint32_t kVddCalLogMagic = 0x54524343u;  // "TRCC"
...
void vdd_cal_log_print() {
    if (!vdd_cal_log_valid()) {
        am_util_stdio_printf(
            "BISen camera TRACE SRAM log INVALID; no results available\n");
        return;
    }
    am_util_stdio_printf(
        "BISen camera TRACE SRAM log: records=%lu capacity=%lu"
    ...
    for (uint32_t i = 0u; i < g_vdd_cal_log.record_count; ++i) {
        ...
        am_util_stdio_printf(
            "BISen camera TRACE retained point #%lu: samples=%lu valid=%lu"
            " code_mean=%lu code_min=%lu code_max=%lu trace_mV=%lu mV"
            " ADC_mode_after=%lu\n",
...
void report_captured_vdd_record(const VddCalibrationRecord &record) {
    if (record.valid_samples == 0u) {
        pp_mark_boot();
        am_util_stdio_printf(
            "BISen camera TRACE retained point #%lu FAILED: no valid ADC samples\n",
            (unsigned long)record.sequence);
    } else {
        am_util_stdio_printf(
            "BISen camera TRACE retained point #%lu: samples=%lu valid=%lu"
            ...
            (unsigned long)record.nominal_vdd_mv);
        am_util_stdio_printf(
            "BISen camera TRACE calibration audit: ADC_mode_after=%u;"
...
[[noreturn]] void run_vdd_calibration_diagnostic() {
    vdd_cal_log_prepare();
    am_util_stdio_printf(
        "BISen camera TRACE offline-capture diagnostic ARMED\n"
...
bool run_bisen_job(bool restored_from_storage) {
    ...
    for (;;) {
        ...
        if (!reported_initial_energy) {
            am_util_stdio_printf(
                "BISen camera initial energy: policy_code=%lu trace_mV=%lu mV"
                " band=%s\n",
                (unsigned long)pp_adc_code(),
                (unsigned long)pp_vcap_mv(), pp_band_name());
        ...
        if (!energy_allows_work) {
            ...
            am_util_stdio_printf(
                "BISen camera wait: policy_code=%lu trace_mV=%lu mV band=%s"
                " phase=%s position=%lu"
...
int main() {
    ...
    am_util_stdio_printf(
        "\nBISen camera/CNN EXTERNAL TRACE EMULATION:"
        " AMAP4PEVB Rev. 1 / apollo4p_evb\n");
    am_util_stdio_printf(
        "BISen camera pins: pixel=J9.10/GPIO15/ADCSE4"
        " TRACE=J9.8/GPIO16/ADCSE3 via 20k/10k divider"
...
#if BISEN_TRACE_CALIBRATION_MODE
    am_util_stdio_printf(
        "BISen camera TRACE CALIBRATION MODE: workload=off MRAM=off"
    ...
    am_util_stdio_printf(
        "BISen camera energy=%s raw-code policy: 1000@%u (2.20 V trace)"
        " 500@%u (2.10 V trace) 100@%u (2.00 V trace) wait<%u"
        " resume=%u (2.10 V trace) sleep<%u (1.90 V trace); hysteresis=none\n",
    ...
    am_util_stdio_printf(
        "EMULATED energy only: policy_code is virtual; trace_mV is the"
        " divided-input reconstruction, NOT physical board VDD\n");
    am_util_stdio_printf(
    ...
    am_util_stdio_printf(
        "BISen camera TRACE estimator: external ADCSE3, discard=1 AVG16"
        " median=3 AVG16"
